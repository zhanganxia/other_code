...
class User(BaseModel,db.Model):
    ...
    id_card = db.Column(db.String(128)) #身份证号
...
houses_facility = db.Table(
    "ih_house_facility",
    db.Column("house_id",db.Integer,db.ForeignKey("ih_house_info_id"),primary_key=True),#房屋编号
...
class House(BaseModel,db.Model):
    ...
    facilities = db.relationship("Facility",secondary=houses_facility) #房屋的设施
...
class Facility(BaseModel,db.Model):
    ...
    name = db.Column(db.String(32),nullable=False) #设施的名字
...
class HouseImage(BaseModel,db.Model):
    ...
    house_id = db.Column(db.Integer,db.ForeignKey("ih_house_info_id"),nullable=False) #房屋编号
...
class Order(BaseModel,db.Model):
    ...
    user_id = db.Column(db.Integer,db.ForeignKey("ih_user_profile.id"),nullable=Flase) #下订单的用户编号
    ...
    comment = db.Column(db.Text) #订单的评论信息或者拒单的原因